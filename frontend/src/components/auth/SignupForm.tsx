...
import { isValidEmail, validatePasswordClient } from '../../utils/validation';
...
export const SignupForm: React.FC<SignupFormProps> = ({ onSubmit, i18n }) => {
  ...
  const validate = (): boolean => {
    ...
    const pwd = validatePasswordClient(password);
    if (!pwd.valid) e.password = dict.t('auth.errors.passwordPolicy');
    if (!confirmPassword) e.confirmPassword = dict.t('auth.errors.required');
    if (password && confirmPassword && password !== confirmPassword) e.confirmPassword = dict.t('auth.errors.passwordMismatch');
    setErrors(e);
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await onSubmit({ email: email.trim().toLowerCase(), password, confirmPassword });
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} style={styles.form} noValidate>
      ...
      <div style={styles.field}>
        ...
        <small style={{ color: '#6b7280' }}>{dict.t('auth.passwordHint')}</small>
        {errors.password && <div style={styles.error}>{errors.password}</div>}
      </div>

      <div style={styles.field}>
        <label htmlFor="confirmPassword" style={styles.label}>{dict.t('auth.confirmPassword')}</label>
        <input
          id="confirmPassword"
          type={showPassword ? 'text' : 'password'}
          autoComplete="new-password"
          ...
          placeholder={dict.t('auth.placeholders.confirmPassword')}
          style={styles.input}
        />
      ...
      <button type="submit" disabled={loading} style={{ ...styles.submit, opacity: loading ? 0.7 : 1 }}>
        {loading ? dict.t('auth.loading') : dict.t('auth.signup')}
      </button>
    </form>
  );
};
